...
const sendMail = (msg:{to?:string, from:string, subject:string, text:string, html?:string, bcc?:[string]}) => {
    if(Meteor.isServer) {
        if(Meteor.settings.private && Meteor.settings.private.SENDGRID_API_KEY) {
            ...
            console.warn('Provide an `SENDGRID_API_KEY` in settings.json to send emails.')
        }
    }
}
